...
document.addEventListener("DOMContentLoaded", async () => {
      ...
      html += `
        <article class="project-card${hiddenClass}" data-cursor="pen" data-category="${item.category}" data-recent="${!!item.recent}" style="animation-delay:${(idx % VISIBLE_INITIAL) * 0.03}s">
          <div class="project-media">
            <img src="${src}" alt="${item.title}" class="real-media" loading="lazy" decoding="async" onerror="this.style.display='none'">
  ...
  function buildVideoGrid(projects) {
    ...
    projects.forEach((item, idx) => {
      ...
      html += `
        <article class="project-card${hiddenClass}" data-cursor="play" data-category="${item.category}" data-recent="${!!item.recent}" data-drive-id="${driveId}" data-aspect="16/9" style="animation-delay:${(idx % VISIBLE_INITIAL) * 0.03}s">
          <div class="project-media">
  ...
  function buildVlogGrid(projects) {
    ...
    projects.forEach((item, idx) => {
      ...
      const poster = item.poster || "";
      html += `
        <article class="project-card vlog-card${orientationClass}${hiddenClass}" data-cursor="play" data-recent="${!!item.recent}" data-drive-id="${driveId}" data-aspect="${item.aspectRatio || "16/9"}" style="animation-delay:${idx * 0.04}s">
  ...
  function buildRecentStrip(data) {
    ...
    const all = [
      ...data.design.map((p) => ({ ...p, type: "design", src: p.imageUrl || p.image, icon: "🖼️" })),
      ...data.video.map((p) => ({ ...p, type: "video", driveId: p.driveId || "", poster: p.poster || "", icon: "🎬" })),
      ...data.vlog.map((p) => ({ ...p, type: "vlog", driveId: p.driveId || "", poster: p.poster || "", icon: "🎥" })),
  ...
  // Google Drive videos can only be played through Drive's own embedded
  // player (a plain <video src> pointed at a Drive link doesn't work), so
  // video/vlog cards open this instead of the img/video lightbox above.
  // The same lightbox shell (overlay, close button, Esc-to-close) is reused
  // so the look and behavior stays identical to the rest of the site.
  function openDriveLightbox(driveId, aspect) {
    document.body.classList.add("lightbox-open");
    lightboxContent.innerHTML = "";
    ...
    iframe.className = "drive-frame" + (aspect === "9/16" ? " is-vertical" : "");
  ...
  function closeLightbox() {
    ...
    lightboxContent.innerHTML = ""; // also stops Drive iframe playback
  }
  ...
  ["designGrid", "videoGrid", "vlogGrid"].forEach((gridId) => {
    ...
    grid.addEventListener("click", (e) => {
      ...
      // Video/vlog cards carry a data-drive-id attribute (possibly empty
      // if that video hasn't been uploaded to Drive yet); design cards don't.
      if (card.dataset.driveId !== undefined) {
        const driveId = card.dataset.driveId;
        if (!driveId) {
          showToastMsg("This video isn't available yet — check back soon");
          return;
        }
        openDriveLightbox(driveId, card.dataset.aspect || "16/9");
        return;
      }
